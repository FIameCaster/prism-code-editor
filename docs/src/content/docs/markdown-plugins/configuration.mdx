...
This page will tell you which configuration options are available for the rehype plugin or marked extension.

## Properties

### `editorsOnly`

When true, code blocks without the `editor` property are ignored, but you can still create code blocks by adding the `editor=false` meta property. This is useful if you're using another plugin to create the code blocks.
...
The [features page](/rehype-marked-plugins/features#custom-render-function) shows an example.

### `inline`

Options for highlighting inline code between backticks. If this option is omitted, inline highlighting will be disabled. The [features page](/rehype-marked-plugins/features#inline-highlighting) explains this feature in more detail.
