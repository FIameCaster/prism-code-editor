...
The only argument to `renderCodeBlock` is a [`CodeBlockOptions{:js}`](/api-reference/ssr/t-codeblockoptions) object. These options include a few properties not present in `EditorOptions`: `lineNumberStart`, `preserveIndent`, and `guideIndents`.
...
[`highlightBracketPairsOnHover(){:js}`](/api-reference/code-blocks/f-highlightbracketpairsonhover) will add highlighting to bracket pairs when hovered. Clicking on a pair keeps it highlighted and clicking anywhere inside the codeblock removes the highlight. It doesn't require rainbow brackets to work. You can try it in any code block on this page.
...
[`highlightTagPairsOnHover(){:js}`](/api-reference/code-blocks/f-highlighttagpairsonhover) is very similar to [`highlightBracketPairsOnHover(){:js}`], but highlights the tagname in matching tags instead. You can try it in the [DOM structure](#dom-structure) code block.

### Adding Overlays

The [`addOverlay(){:js}`](/api-reference/utils/f-addoverlay) utility can be used to add overlays to code blocks as well. The only difference to adding overlays to editors is that code blocks don't have `pointer-events: none{:css}` and `user-select: none{:css}`.

### Hover Descriptions

If you have complicated code you want to describe, but without cluttering the code up with comments, you can show descriptions on hover instead with [`addHoverDescription(){:js}`](/api-reference/code-blocks/f-addhoverdescription). Below is a very simple example. Try hovering a string token.
...
To ensure the tooltip doesn't get too big and overflows, `max-width` and `max-height` are added as inline styles to the tooltip. If you want to set your own `max-width` or `max-height`, you can use the third argument to `addHoverDescription(){:js}`. The values you provide are added as arguments to the CSS [`min(){:css}`](https://developer.mozilla.org/en-US/docs/Web/CSS/min) function, so multiple values can be comma separated.
