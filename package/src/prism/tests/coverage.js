...
import { dependencyGraph } from './helper/lang-info.js';
...
describe('Pattern test coverage', () => {
	...
	describe('Register all patterns', () => {
		it('all', async function () {
			...
			await loadLanguages(Object.keys(dependencyGraph));
			globalize();
		});
	});
		...
		for (const [languageIdentifier, files] of loadAllTests()) {
			it(languageIdentifier, async function () {
				...
				for (const filePath of files) {
					try {
						await runTestCase(languageIdentifier, filePath, 'none', globalize);
					} catch (error) {
	...
	describe('Coverage', () => {
		for (const language in dependencyGraph) {
			describe(language, () => {
