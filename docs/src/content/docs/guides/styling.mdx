...
To change the current theme with the setups, use the [`setOptions()`](/api-reference/index/i-prismeditor#setoptions) method. Updating the theme is asynchronous, and the old theme is used until the new one loads.

<Editor language="javascript" value={`editor.setOptions({
	theme: "night-owl"
})`} />
...
<Editor language="javascript" value={`import { registerTheme } from "prism-code-editor/themes"
...
You can also import themes into your JavaScript with [`loadTheme()`](/api-reference/themes/f-loadtheme) which is used by theme switcher in the playground.

<Editor language="javascript" value={`import { loadTheme } from "prism-code-editor/themes"
...
- `prism-code-editor/search.css`. Styles for [`searchWidget()`](/api-reference/search/f-searchwidget).
- `prism-code-editor/copy-button.css`: Styles for [`copyButton()`](/api-reference/copy-button/f-copybutton).
- `prism-code-editor/invisivles.css`: Styles for [`showInvisibles()`](/api-reference/search/f-showinvisibles).
- `prism-code-editor/code-folding.css`: Styles for [`readOnlyCodeFolding()`](/api-reference/code-folding/f-readonlycodefolding).
- `prism-code-editor/autocomplete.css`: Styles for [`autoComplete()`](/api-reference/autocomplete/f-autocomplete).
...
<Editor language="html" value={`<div class="prism-code-editor language-[lang] [state-classes]">
...
There are many classes added to `.prism-code-editor` you can yse to style the editor depending on its state:
