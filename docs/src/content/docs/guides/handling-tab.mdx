...
The [`defaultCommands()`](/api-reference/commands/f-defaultcommands) extension binds the Tab key to indenting code. Users can at any time toggle tab capturing with Ctrl+M / Ctrl+Shift+M (Mac) whenever an editor is focused, but if a keyboard navigator is unaware of this, they're effectively trapped in the editor.
...
<Editor language="typescript" value={`import { setIgnoreTab, ignoreTab } from "prism-code-editor/commands"
