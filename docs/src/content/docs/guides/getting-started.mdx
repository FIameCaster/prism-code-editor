...
import "../../../components/languages/bash+html+css+ts.astro"
...
<Tabs>
  <TabItem label="npm">
		<Editor language="bash" value={"npm install prism-code-editor"} />
  </TabItem>
  <TabItem label="pnpm">
		<Editor language="bash" value={"pnpm add prism-code-editor"} />
  </TabItem>
  <TabItem label="yarn">
		<Editor language="bash" value={"yarn add prism-code-editor"} />
  </TabItem>
</Tabs>

## Basic usage

This library provides multiple ways to create editors. Perhaps the easiest is to use one of the 3 different setups: [`minimalEditor`](/api-reference/setups/f-minimaleditor), [`basicEditor`](/api-reference/setups/f-basiceditor), or [`readOnlyEditor`](/api-reference/setups/f-readonlyditor). These will automatically import the necessary styles and scope them with a shadow root, add various extensions, and import all language specific behavior. There are multiple downsides to using the setups, but they're a viable option for anyone who wants an editor that works well out of the box.

<Editor language="javascript" value={`import { basicEditor } from "prism-code-editor/setups"
...
<Aside type="note">
	You might want to add `display: grid` to your container element.
</Aside>

## Configuration

The second argument (type [`EditorOptions`](/api-reference/index/types/editoroptions)) to the setups allows some basic configuration of the created editor. Below is a list of the allowed properties.

- `language`: Language used for syntax highlighting. Defaults to `text`
- `tabSize`: Tab size used for indentation. Defaults to `2`.
- `insertSpaces`: Whether the editor should insert spaces for indentation. Defaults to `true`. Requires the `defaultCommands()` extension to work.
...
- `rtl`: Whether the editor uses right to left directionality. Defaults to `false`. Requires extra CSS from `prism-code-editor/rtl-layout.css` to work.
...
<Editor language="typescript" value={`import { addBasicEditor, PrismEditorElement } from "prism-code-editor/web-component"
...
<Editor language="html" value={`<style>
	prism-editor {
		display: grid;
	}
</style>
