...
It's not an uncommon problem wanting to place tooltips relative to another element in the editor. The [`addTooltip()`](/api-reference/tooltips/f-addtooltip) utility can be used to make this easier. This utility is used by [`autoComplete()`](/api-reference/autocomplete/f-autocomplete) to position the list.
...
<Editor language="typescript" readOnly value={`import { addTooltip } from "prism-code-editor/tooltips"
...
<Editor language="css" value={`.my-tooltip {
  position: sticky;
  left: 0.5em;
  right: 0.5em;
  padding: 0.4em;
