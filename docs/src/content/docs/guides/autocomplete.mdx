...
To create snippets, use the [`insert`](/api-reference/autocomplete/i-completion#insert) property along with [`tabStops`](/api-reference/autocomplete/i-completion#tabstops). If there are line feeds in `insert`, the indentation will be preserved. Tabs are replaced by spaces with `options.insertSpaces{:js}` set to true.
